...
### Subscriptions evaluated by AzSDK
Below query is useful to get an idea of overall AzSDK usage in your subscriptions. It displays number of subscriptions scanned by AzSDK (through CA runbook/SpotCheck (Adhoc scan)/VSO task) in last 7 days. 
...
### Resources evaluated by AzSDK
Below query is useful to get an idea of overall AzSDK usage in your subscriptions. It displays number of resources scanned by AzSDK (through CA runbook/SpotCheck (Adhoc scan)/VSO task) in last 7 days.  
...
![AIGraph_AzSDK_Usage.jpg](/Images/06_AIGraph_AzSDK_Usage.jpg)

### AzSDK Version statistics

#### _CA AzSDK module version usage_

Below query is useful to check AzSDK module version used by CA installed in subscriptions.
...
| summarize CurrentRunningCAAzSDKVersion=count() by Version
| render piechart
```

#### _SpotCheck AzSDK module version usage_

Below query is useful to check AzSDK module version used by developers in local scan.
...
| summarize ControlsScanned = count() by Version = tostring(customDimensions.ScannerVersion), SubId = tostring(customDimensions.SubscriptionId)
| distinct Version,SubId,ControlsScanned
| summarize AzSDKVersion=count() by Version
| render piechart
```
...
| summarize arg_max(timestamp, *) by tostring(customDimensions.SubscriptionId)
| project tostring(customDimensions.SubscriptionId),tostring(customDimensions.AzSDKRunbookVersion)
)
on customDimensions_SubscriptionId
| extend RunbookVersion = iff(isempty(customDimensions_AzSDKRunbookVersion)==true,"< 2.5.0",customDimensions_AzSDKRunbookVersion)
...
![AIGraph_AzSDK_ModuleVersion.jpg](/Images/06_AIGraph_AzSDK_ModuleVersion.jpg)

![AIGraph_AzSDK_RunbookVersion.jpg](/Images/06_AIGraph_AzSDK_RunbookVersion.jpg)
...
![AIGraph_AzSDK_ScanResultDistribution.jpg](/Images/06_AIGraph_AzSDK_ScanResultDistribution.jpg)
