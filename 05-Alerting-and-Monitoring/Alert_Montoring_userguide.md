...
3. For the CC runbooks, the OMS workspace info specified in the input parameters of the [Continuous Compliance setup script](../04-Continous-Assurance/Continuous_Assurance_userguide.md).  
...
5. Common useful queries for alerting, etc.

The solution also configures OMS-based alerts (with similar alerting behavior and consideration of alert severity as the Set-AzSDKAlerts script does) and runbooks for auto-healing.
...
Create a copy of the following file (from the AzSDK setup folder under [TBD]):
[TBD:JSON file path]
...
Notes: 
- Any spaces in the 'location' parameter should be removed (e.g., make it 'SouthEastAsia' instead of 'SouthEast Asia').
...
2. Adds multiple "Saved Searches", 
...
appQuery	|Change the resourcegroup name "appQuery": { "value": " ResourceGroup=\"xygappresourcegroupname\"" } **[TODO] Edit query to support multiple resource groups?**|
...
At this point, we can confirm that the ad hoc security scan we ran in Steps 4, 5 above generated data for the application control compliance by looking at the view (by clicking the tile above).
...
Lastly, if you also setup continuous compliance (AzSDK CC runbooks), you will also see activity in the first blade as shown below:
...
TODO - Add some more guidance on setting up alerts/other functions such as Autoheal, etc.
